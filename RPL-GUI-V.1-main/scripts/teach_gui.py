...
class teach_ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.filepath2 = rp.get_path('table_server')
    ...
    def dropdown_setup(self):
        ...
        # TODO: add topic-finder with given output broadcast to add to the list, allowing the relative pose broadcaster to auto subscribe
        ...
        # self.pointSetter.setMenu(dropdown_menu)
        # self.pointSetter.setPopupMode(QtWidgets.QToolButton.InstantPopup)
    ...
    def setupVis(self):
        ...
        reader.readFile( config, self.filepath2+ "/rviz/gui_vis.rviz" )
    ...
    def recordPose(self):
        ...
        rot,th = quaternion_to_axis_angle(rot)
        rot *=th
        ...
        if self.vectorSpaceSetter.text() == 'Cartesian':
            self.pose_motion_handler.add_pose(pose_name,pose,'cart',frame_id)
        else:
            self.pose_motion_handler.add_pose(pose_name,pose,'joint',frame_id, self.joint_pose)
    ...
    # def customActionCB(self):
    #     self.custom_action_ui = CustomActionWidget(self.filepath)
    #     self.custom_action_ui.exec_()
    ...
    def motionDropDown(self):
        ...
        # for other robots - e.g. the KUKA IIWA, we can have a purely force based controller. This can be an option in the future when loading in a custom robot, creating a coupled config file.
    ...
    def handleMotionSelected(self,option):        
        if option == 'custom':
            ...
            # TODO
            #if option == 'cartesian with force':
            #   self.additional = force information
    ...
    def saveMotionsButton(self):
        ...
        if command_type.contains('force'):
    ...
    def add_vis_widget(self, name, element_name,label_, data_type = 'img', n_var = None):
        # get the widget based on its name, then calling the different viewing widgets
